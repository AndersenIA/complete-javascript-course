...
// EVERY: every element must satisfy the condition
...
console.log('--------DOG BOTH ACTIVITIES---------');
...
const allWeights = breeds
  .map(breeds => breeds.averageWeight)
  .reduce((acc, bre) => acc + bre, 0);
console.log(allWeights / breeds.length > 10);
console.log(breeds.every(breed => breed.averageWeight >= 10));

const activeBreeds = breeds.filter(breeds => breeds.activities.length >= 3);
console.log(activeBreeds);
console.log(breeds.some(breed => breed.activities.length >= 3));

const heaviestFetchBreed = breeds
  .filter(breeds => breeds.activities.includes('fetch'))
  .map(weight => weight.averageWeight);

console.log(Math.max(...heaviestFetchBreed));
// console.log(Math.max(...heaviestFetchBreed.averageWeight));
*/
...
/*
// reverse is a destructive method. That will change the original. But using toReversed() will keep the original array.
console.log(movements);
// const reversedMov = movements.slice().reverse(); // This also works.
const reversedMov = movements.toReversed();
console.log(reversedMov);

// toSorted (sort), toSpliced (splice)

// movements[1] = 2000; // mutates the original
...
const { deposits, withdrawals } = accounts
  ...
  .reduce(
    (sums, cur) => {
      // cur > 0 ? (sums.deposits += cur) : (sums.withrdrawals += cur);
...
const recFood = weight => weight ** 0.75 * 28;

// 1.
/*1. Loop over the array containing dog objects, and for each dog, calculate the recommended food portion (recFood) and add it to the object as a new property. Do NOT create a new array, simply loop over the array (We never did this before, so think about how you can do this without creating a new array). 
dogs.forEach(dog => (dog.recommendedFood = Math.floor(recFood(dog.weight))));
...
dogs.forEach(dog => {
  if (dog.owners.includes('Sarah')) {
    ...
    dog.curFood > dog.recommendedFood
      ? console.log('Sarahs dog eats too much food.')
      : console.log('Sarahs dog does not eat enough food.');
  }
});
...
console.log(
  `${ownersTooLittle.flat().join(' and ')}'s dogs does not eat enough food`
);
...
// dogs.forEach(dog => {
//   if (dog.curFood === dog.recommendedFood) {
//     console.log(`${dog.owners}'s dog eats exactly right`);
//   }
// });

const anyExact = dogs.some(dog => dog.curFood === dog.recommendedFood);
console.log('Any dog eating exactly the recommended amount?', anyExact);
...
dogs.forEach(dog => {
  if (
    ...
    console.log(`${dog.owners}'s dog eats an OKAY amount of food`);
...
// const grouping = function ({ recommendedFood }) {
...
const result = Object.groupBy(dogs, ({ curFood, recommendedFood }) => {
